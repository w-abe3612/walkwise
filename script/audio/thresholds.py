...
"""STEP4 typed source scaffold for script/audio/thresholds.py.
...
STEP4_PUBLIC_CONTRACTS: tuple[tuple[str, str, str], ...] = (
    ('TASK-AUDIO-002', 'AudioThresholds.load()/validate_approval()', 'provisional値を読込み、実測不足でapprovedを拒否する。'),
)
...
def _step4_unimplemented(symbol: str) -> None:
    raise NotImplementedError(f"STEP4 source scaffold is not implemented: {symbol} (script/audio/thresholds.py)")
...
class AudioThresholds:
    ...
    def load(self) -> Any:
        ...
        Public contract: ``AudioThresholds.load()/validate_approval()``.
        """
        _step4_unimplemented('AudioThresholds.load')
    def validate_approval(self) -> Any:
        """provisional値を読込み、実測不足でapprovedを拒否する。

        Public contract: ``AudioThresholds.load()/validate_approval()``.
        """
        _step4_unimplemented('AudioThresholds.validate_approval')
