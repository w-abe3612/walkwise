...
"""STEP4 typed source scaffold for script/audio/measurements.py.
...
STEP4_PUBLIC_CONTRACTS: tuple[tuple[str, str, str], ...] = (
    ('TASK-AUDIO-002', 'AudioMeasurementAdapter.check_runtime()/measure(path)', 'ffprobe/ffmpeg利用可能性確認と測定を分離する。'),
)
...
def _step4_unimplemented(symbol: str) -> None:
    raise NotImplementedError(f"STEP4 source scaffold is not implemented: {symbol} (script/audio/measurements.py)")
...
class AudioMeasurementAdapter:
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        self._args = args
        self._kwargs = kwargs
    def check_runtime(self) -> Any:
        """ffprobe/ffmpeg利用可能性確認と測定を分離する。

        Public contract: ``AudioMeasurementAdapter.check_runtime()/measure(path)``.
        """
        _step4_unimplemented('AudioMeasurementAdapter.check_runtime')
    def measure(self, path: Any) -> Any:
        """ffprobe/ffmpeg利用可能性確認と測定を分離する。

        Public contract: ``AudioMeasurementAdapter.check_runtime()/measure(path)``.
        """
        _step4_unimplemented('AudioMeasurementAdapter.measure')
