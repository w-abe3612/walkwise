...
"""STEP4 typed source scaffold for script/audio/synthesis.py.
...
STEP4_PUBLIC_CONTRACTS: tuple[tuple[str, str, str], ...] = (
    ('TASK-AUDIO-001', 'SegmentSynthesizer.synthesize(script, profile) -> list[SegmentAudio]', 'TTS clientを通じてsegment/part WAVを生成する。'),
)
...
def _step4_unimplemented(symbol: str) -> None:
    raise NotImplementedError(f"STEP4 source scaffold is not implemented: {symbol} (script/audio/synthesis.py)")
...
class SegmentSynthesizer:
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        self._args = args
        self._kwargs = kwargs
    def synthesize(self, script: Any, profile: Any) -> list[SegmentAudio]:
        """TTS clientを通じてsegment/part WAVを生成する。

        Public contract: ``SegmentSynthesizer.synthesize(script, profile) -> list[SegmentAudio]``.
        """
        _step4_unimplemented('SegmentSynthesizer.synthesize')
