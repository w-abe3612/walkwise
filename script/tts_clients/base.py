...
"""STEP4 typed source scaffold for script/tts_clients/base.py.
...
STEP4_PUBLIC_CONTRACTS: tuple[tuple[str, str, str], ...] = (
    ('TASK-TTS-001', 'TTSClient Protocol: check_connectivity/list_speakers/synthesize', '疎通、一覧、合成を分離する。'),
    ('TASK-TTS-001', 'TTSClientError(code, engine_detail)', '固有例外を共通error codeへ変換する。'),
)
...
def _step4_unimplemented(symbol: str) -> None:
    raise NotImplementedError(f"STEP4 source scaffold is not implemented: {symbol} (script/tts_clients/base.py)")

class TTSClientError(RuntimeError):
    """Public error placeholder declared by the STEP2 contract."""

class TTSClient(Protocol):
    """Protocol fixed by STEP2 for script/tts_clients/base.py."""
    def check_connectivity(self) -> Any:
        """疎通、一覧、合成を分離する。 Contract: TTSClient Protocol: check_connectivity/list_speakers/synthesize"""
        ...
