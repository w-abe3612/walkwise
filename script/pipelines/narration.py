...
"""STEP4 typed source scaffold for script/pipelines/narration.py.
...
STEP4_PUBLIC_CONTRACTS: tuple[tuple[str, str, str], ...] = (
    ('TASK-NARRATION-001', 'NarrationPipeline.simplify/adapt_for_audio/apply_character', '各段階で新しいScriptDocumentを生成しtext由来を保持する。'),
    ('TASK-NARRATION-001', 'build_verified_script(...) -> ScriptDocument', 'fact checkとsemantic review通過後だけverifiedを作る。'),
)
...
def _step4_unimplemented(symbol: str) -> None:
    raise NotImplementedError(f"STEP4 source scaffold is not implemented: {symbol} (script/pipelines/narration.py)")
...
class NarrationPipeline:
    ...
    def simplify(self) -> Any:
        """各段階で新しいScriptDocumentを生成しtext由来を保持する。

        Public contract: ``NarrationPipeline.simplify/adapt_for_audio/apply_character``.
        """
        _step4_unimplemented('NarrationPipeline.simplify')
    def adapt_for_audio(self) -> Any:
        """各段階で新しいScriptDocumentを生成しtext由来を保持する。

        Public contract: ``NarrationPipeline.simplify/adapt_for_audio/apply_character``.
        """
        _step4_unimplemented('NarrationPipeline.adapt_for_audio')
    def apply_character(self) -> Any:
        """各段階で新しいScriptDocumentを生成しtext由来を保持する。

        Public contract: ``NarrationPipeline.simplify/adapt_for_audio/apply_character``.
        """
        _step4_unimplemented('NarrationPipeline.apply_character')

def build_verified_script(*args: Any, **kwargs: Any) -> ScriptDocument:
    """fact checkとsemantic review通過後だけverifiedを作る。

    Public contract: ``build_verified_script(...) -> ScriptDocument``.
    """
    _step4_unimplemented('build_verified_script')
