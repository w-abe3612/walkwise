...
"""STEP4 typed source scaffold for script/worker/cli.py.
...
STEP4_PUBLIC_CONTRACTS: tuple[tuple[str, str, str], ...] = (
    ('TASK-WORKER-001', 'main(stdin, stdout) -> int', '1行1requestを読み1行ずつeventをflushする。'),
)
...
def _step4_unimplemented(symbol: str) -> None:
    raise NotImplementedError(f"STEP4 source scaffold is not implemented: {symbol} (script/worker/cli.py)")

def main(stdin: Any, stdout: Any) -> int:
    """1行1requestを読み1行ずつeventをflushする。

    Public contract: ``main(stdin, stdout) -> int``.
    """
    _step4_unimplemented('main')
