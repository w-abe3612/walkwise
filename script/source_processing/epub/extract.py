...
"""STEP4 typed source scaffold for script/source_processing/epub/extract.py.
...
STEP4_PUBLIC_CONTRACTS: tuple[tuple[str, str, str], ...] = (
    ('TASK-EPUB-001', 'EpubTextExtractor.extract(path) -> EpubExtractionReport', 'DRM-free EPUBをspine順で抽出する。'),
    ('TASK-EPUB-001', 'detect_drm_or_encryption(path) -> bool', '暗号化・DRM疑いを処理前に検出し拒否する。'),
)
...
def _step4_unimplemented(symbol: str) -> None:
    raise NotImplementedError(f"STEP4 source scaffold is not implemented: {symbol} (script/source_processing/epub/extract.py)")
...
class EpubTextExtractor:
    ...
    def extract(self, path: Any) -> EpubExtractionReport:
        ...
        """
        _step4_unimplemented('EpubTextExtractor.extract')

def detect_drm_or_encryption(path: Any) -> bool:
    """暗号化・DRM疑いを処理前に検出し拒否する。

    Public contract: ``detect_drm_or_encryption(path) -> bool``.
    """
    _step4_unimplemented('detect_drm_or_encryption')
