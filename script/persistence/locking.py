...
"""STEP4 typed source scaffold for script/persistence/locking.py.
...
STEP4_PUBLIC_CONTRACTS: tuple[tuple[str, str, str], ...] = (
    ('TASK-FILE-001', 'ProjectLock.acquire(project_root: Path) -> ContextManager[ProjectLock]', 'Project単位排他lockを取得・解放する。'),
)
...
def _step4_unimplemented(symbol: str) -> None:
    raise NotImplementedError(f"STEP4 source scaffold is not implemented: {symbol} (script/persistence/locking.py)")
...
class ProjectLock:
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        self._args = args
        self._kwargs = kwargs
    def acquire(self, project_root: Path) -> ContextManager[ProjectLock]:
        """Project単位排他lockを取得・解放する。

        Public contract: ``ProjectLock.acquire(project_root: Path) -> ContextManager[ProjectLock]``.
        """
        _step4_unimplemented('ProjectLock.acquire')
