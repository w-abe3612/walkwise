...
"""STEP4 typed source scaffold for script/profiles/characters.py.
...
STEP4_PUBLIC_CONTRACTS: tuple[tuple[str, str, str], ...] = (
    ('TASK-PROFILE-001', 'CharacterProfileRepository.load/select', '承認状態と用途を検証して選択する。'),
)
...
def _step4_unimplemented(symbol: str) -> None:
    raise NotImplementedError(f"STEP4 source scaffold is not implemented: {symbol} (script/profiles/characters.py)")
...
class CharacterProfileRepository:
    ...
    def load(self) -> Any:
        ...
        Public contract: ``CharacterProfileRepository.load/select``.
        """
        _step4_unimplemented('CharacterProfileRepository.load')
    def select(self) -> Any:
        """承認状態と用途を検証して選択する。

        Public contract: ``CharacterProfileRepository.load/select``.
        """
        _step4_unimplemented('CharacterProfileRepository.select')
