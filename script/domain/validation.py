...
"""STEP4 typed source scaffold for script/domain/validation.py.
...
STEP4_PUBLIC_CONTRACTS: tuple[tuple[str, str, str], ...] = (
    ('TASK-DOMAIN-001', 'canonicalize_output_formats(values: Sequence[str]) -> tuple[str, ...]', 'mp3→text順へ正規化し、空・未知・重複を拒否する。'),
    ('TASK-DOMAIN-001', 'validate_build_request(formats, voice_profile_id) -> None', 'mp3を含む場合だけvoice必須とする。'),
)
...
def _step4_unimplemented(symbol: str) -> None:
    raise NotImplementedError(f"STEP4 source scaffold is not implemented: {symbol} (script/domain/validation.py)")

def canonicalize_output_formats(values: Sequence[str]) -> tuple[str, ...]:
    """mp3→text順へ正規化し、空・未知・重複を拒否する。

    Public contract: ``canonicalize_output_formats(values: Sequence[str]) -> tuple[str, ...]``.
    """
    _step4_unimplemented('canonicalize_output_formats')

def validate_build_request(formats: Any, voice_profile_id: Any) -> None:
    """mp3を含む場合だけvoice必須とする。

    Public contract: ``validate_build_request(formats, voice_profile_id) -> None``.
    """
    _step4_unimplemented('validate_build_request')
