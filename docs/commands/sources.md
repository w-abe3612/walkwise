...
Source登録、immutable原本、状態遷移の検証に使用する正式な実行入口を定義する。
...
現在の存在有無は[`CURRENT_STATE.md`](CURRENT_STATE.md)を正本とする。`TASK-SOURCE-001`は
本実装済みであり、対象8ケース(TC-SOURCE-001-01〜08)はpassする。
...
### Docker正式
```powershell
docker compose run --rm test python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_source_service.py tests/test_source_status_transitions.py
```
