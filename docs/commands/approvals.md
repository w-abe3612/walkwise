...
4段階承認、差し戻し、revision変更時の無効化検証に使用する正式な実行入口を定義する。
...
現在の存在有無は[`CURRENT_STATE.md`](CURRENT_STATE.md)を正本とする。`TASK-APPROVAL-001`は
本実装済みであり、対象10ケース(TC-APPROVAL-001-01〜10)はpassする。

## 5. 収集・型確認

```powershell
python -m pytest --collect-only -q tests/test_approval_invalidation.py tests/test_approval_workflow.py
```
...
### Host補助
```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_approval_invalidation.py tests/test_approval_workflow.py
```
### Docker正式
```powershell
docker compose run --rm test python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_approval_invalidation.py tests/test_approval_workflow.py
```
