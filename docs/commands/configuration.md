...
設定、共通error、秘密値除外、loggingの検証に使用する正式な実行入口を定義する。
...
```powershell
python -m pytest --collect-only -q tests/test_core_config.py tests/test_core_errors.py tests/test_core_logging.py
```
...
### Host補助
```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_core_config.py tests/test_core_errors.py tests/test_core_logging.py
```
### Docker正式
```powershell
docker compose run --rm test python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_core_config.py tests/test_core_errors.py tests/test_core_logging.py
```
