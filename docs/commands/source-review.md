...
抽出結果の人間修正、revision、再承認要求の検証に使用する正式な実行入口を定義する。
...
### Docker正式
```powershell
docker compose run --rm test python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_source_review_service.py
```
