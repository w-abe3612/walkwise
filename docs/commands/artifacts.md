...
Artifact登録、追記専用version、一覧取得の検証に使用する正式な実行入口を定義する。
...
現在の存在有無は[`CURRENT_STATE.md`](CURRENT_STATE.md)を正本とする。`TASK-ARTIFACT-001`は
本実装済みであり、対象9ケース(TC-ARTIFACT-001-01〜09)はpassする。
...
### Docker正式
```powershell
docker compose run --rm test python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_artifact_service.py
```
