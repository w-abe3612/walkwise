...
旧形式の読込、優先順位、変換reportの検証に使用する正式な実行入口を定義する。
...
```powershell
python -m pytest --collect-only -q tests/test_legacy_input_priority.py tests/test_legacy_migration.py
```
...
### Host補助
```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_legacy_input_priority.py tests/test_legacy_migration.py
```
### Docker正式
```powershell
docker compose run --rm test python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_legacy_input_priority.py tests/test_legacy_migration.py
```
