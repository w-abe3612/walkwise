...
依存impact分析と部分再生成計画の検証に使用する正式な実行入口を定義する。
...
### Docker正式
```powershell
docker compose run --rm test python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_impact_analysis.py tests/test_regeneration_plan.py
```
