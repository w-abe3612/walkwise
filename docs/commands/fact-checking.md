...
claim、evidence、source参照、fact-checkingの検証に使用する正式な実行入口を定義する。
...
```powershell
python -m pytest --collect-only -q tests/test_claim_validation.py tests/test_claims_pipeline.py
```
...
### Host補助
```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_claim_validation.py tests/test_claims_pipeline.py
```
### Docker正式
```powershell
docker compose run --rm test python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_claim_validation.py tests/test_claims_pipeline.py
```
