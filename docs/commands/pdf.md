...
PDF直接抽出とOCR fallback判定の検証に使用する正式な実行入口を定義する。
...
現在の存在有無は[`CURRENT_STATE.md`](CURRENT_STATE.md)を正本とする。`TASK-PDF-001`は
本実装済みであり、対象10ケース(TC-PDF-001-01〜10)はpassする。採用ライブラリ
(PyMuPDF/pypdf)とpage品質閾値は`pdf-direct-text-extraction.md`が明記するとおり
`provisional`であり、代表PDFでのbenchmark実施後に確定する。

## 5. 収集・型確認

```powershell
python -m pytest --collect-only -q tests/test_pdf_direct_extraction.py tests/test_pdf_extraction_fallback.py
```
...
### Host補助
```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_pdf_direct_extraction.py tests/test_pdf_extraction_fallback.py
```
### Docker正式
```powershell
docker compose run --rm test python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_pdf_direct_extraction.py tests/test_pdf_extraction_fallback.py
```
