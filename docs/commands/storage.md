...
Project配置、相対path、atomic write、lockの検証に使用する正式な実行入口を定義する。
...
```powershell
python -m pytest --collect-only -q tests/test_atomic_file_write.py tests/test_persistence_paths.py tests/test_project_locking.py
```
...
### Host補助
```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_atomic_file_write.py tests/test_persistence_paths.py tests/test_project_locking.py
```
### Docker正式
```powershell
docker compose run --rm test python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_atomic_file_write.py tests/test_persistence_paths.py tests/test_project_locking.py
```
