...
Job状態遷移、queue、cancel、retry、stale recoveryの検証に使用する正式な実行入口を定義する。
...
- `tests/test_job_lifecycle.py` — 現在のダンプでは欠落
- `tests/test_job_queue.py` — 現在のダンプでは欠落
- `tests/test_stale_job_recovery.py` — 現在のダンプでは欠落
...
### Docker正式
```powershell
docker compose run --rm test python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_job_lifecycle.py tests/test_job_queue.py tests/test_stale_job_recovery.py
```
