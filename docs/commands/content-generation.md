...
# content-generation コマンド

## 1. 目的

資料分析、coverage、curriculum、原稿、narration変換の検証に使用する正式な実行入口を定義する。
...
```powershell
python -m pytest --collect-only -q tests/test_chapter_spec_schema.py tests/test_coverage_map.py tests/test_curriculum_pipeline.py tests/test_draft_generation.py tests/test_narration_transformations.py tests/test_script_schema.py tests/test_source_analysis_pipeline.py tests/test_verified_script_pipeline.py
```
