...
| 項目 | 型 | 必須 |
|---|---|---|
| 出力形式 | チェックボックス(MP3、テキスト) | 1件以上必須 |
| speaker/style | 選択式 | 必須 |
| speed等の音声parameter | スライダー(`09-voice-profile-schema.md`の`speed_scale`等の最小項目) | 任意(既定値あり) |

## 5. 操作

- speaker/styleと音声parameterで試聴する
...
| disabled | EPUB・COEIROINK・M4Bの選択肢は理由付きでdisabled表示する(`19-application-scope-and-mvp.md`のMVP対象外) |

## 8. validation

出力形式が1件以上選択され、speaker/styleが選択されるまで「制作開始」ボタンを
disabledにする。承認ゲート未充足の場合、「制作開始」実行時にIPCエラー
...
- EPUB、COEIROINK、M4Bの表示(理由付きdisabledとして将来機能を明確に除外する)
- 出力プロファイルの保存・複製
