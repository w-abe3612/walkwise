...
depends_on:
- TASK-SOURCE-001
contract_ref: docs/test-cases/TASK-RIGHTS-001-rights-license-and-distribution-gate.md
preparation_ref: docs/spec-proposals/task/111_task-rights-001-rights-license-and-distribution-gate.md
spec_refs:
...
---
# TASK-RIGHTS-001 権利・ライセンス・配布gate
...
1. 承認済み仕様・DB・画面文書
2. `docs/test-cases/TASK-RIGHTS-001-rights-license-and-distribution-gate.md`
...
```powershell
python -m pytest --collect-only -q tests/test_rights_gate.py tests/test_credit_manifest.py
```

### 外部接続なしのPython対象テスト

```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_rights_gate.py tests/test_credit_manifest.py
```
