...
depends_on:
- TASK-DB-001
contract_ref: docs/test-cases/TASK-DB-002-repositories-and-unit-of-work.md
preparation_ref: docs/spec-proposals/task/108_task-db-002-repositories-and-unit-of-work.md
spec_refs:
...
---
# TASK-DB-002 Repository・transaction境界
...
1. 承認済み仕様・DB・画面文書
2. `docs/test-cases/TASK-DB-002-repositories-and-unit-of-work.md`
...
```powershell
python -m pytest --collect-only -q tests/test_repositories.py tests/test_unit_of_work.py
```

### 外部接続なしのPython対象テスト

```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_repositories.py tests/test_unit_of_work.py
```
