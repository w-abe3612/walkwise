...
- TASK-DEV-001
- TASK-CORE-001
contract_ref: docs/test-cases/TASK-CORE-002-identifiers-hashing-and-structured-files.md
preparation_ref: docs/spec-proposals/task/104_task-core-002-identifiers-hashing-and-structured-files.md
spec_refs:
...
1. 承認済み仕様・DB・画面文書
2. `docs/test-cases/TASK-CORE-002-identifiers-hashing-and-structured-files.md`
...
```powershell
python -m pytest --collect-only -q tests/test_identifiers.py tests/test_hashing.py tests/test_serialization.py
```

### 外部接続なしのPython対象テスト

```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_identifiers.py tests/test_hashing.py tests/test_serialization.py
```
