...
- TASK-APPROVAL-001
- TASK-ARTIFACT-001
contract_ref: docs/test-cases/TASK-PIPELINE-001-dependency-impact-and-partial-regeneration.md
preparation_ref: docs/spec-proposals/task/132_task-pipeline-001-dependency-impact-and-partial-regeneration.md
spec_refs:
...
1. 承認済み仕様・DB・画面文書
2. `docs/test-cases/TASK-PIPELINE-001-dependency-impact-and-partial-regeneration.md`
...
```powershell
python -m pytest --collect-only -q tests/test_impact_analysis.py tests/test_regeneration_plan.py
```

### 外部接続なしのPython対象テスト

```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_impact_analysis.py tests/test_regeneration_plan.py
```
