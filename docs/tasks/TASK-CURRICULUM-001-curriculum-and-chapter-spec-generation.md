...
- TASK-AI-003
- TASK-APPROVAL-001
contract_ref: docs/test-cases/TASK-CURRICULUM-001-curriculum-and-chapter-spec-generation.md
preparation_ref: docs/spec-proposals/task/127_task-curriculum-001-curriculum-and-chapter-spec-generation.md
spec_refs:
...
---
# TASK-CURRICULUM-001 topic map・curriculum・章仕様生成
...
1. 承認済み仕様・DB・画面文書
2. `docs/test-cases/TASK-CURRICULUM-001-curriculum-and-chapter-spec-generation.md`
...
```powershell
python -m pytest --collect-only -q tests/test_curriculum_pipeline.py tests/test_chapter_spec_schema.py
```

### 外部接続なしのPython対象テスト

```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_curriculum_pipeline.py tests/test_chapter_spec_schema.py
```
