...
depends_on:
- TASK-ENV-001
contract_ref: docs/test-cases/TASK-DESKTOP-001-electron-vue-scaffold-and-ipc-security.md
preparation_ref: docs/spec-proposals/task/143_task-desktop-001-electron-vue-scaffold-and-ipc-security.md
spec_refs:
...
1. 承認済み仕様・DB・画面文書
2. `docs/test-cases/TASK-DESKTOP-001-electron-vue-scaffold-and-ipc-security.md`
...
|---|---|---|
| `electron/main/index.ts` | `createMainWindow(): BrowserWindow` | 安全なwebPreferencesでrendererを起動する。 |
| `electron/preload/index.ts` | `window.walkwise API` | 許可済みIPCだけを型付きで公開する。 |
| `electron/renderer/main.ts` | `renderer bootstrap` | Node APIへ直接アクセスせずVueを起動する。 |
| `package.json` | `scripts/toolchain` | build/test/typecheck/packageの入口を固定する。 |
...
|---|---|---|---|---|---|---|---|
| TC-DESKTOP-001-01 | P0 | static | webPreferences | main window作成 | 設定を検査 | nodeIntegration=false, contextIsolation=true, sandbox方針を満たす | `electron/tests/main_security.test.ts` |
| TC-DESKTOP-001-02 | P0 | unit | preload allowlist | 未許可channelを呼ぶ | API | rendererから送信できない | `electron/tests/preload_contract.test.ts` |
| TC-DESKTOP-001-03 | P0 | static | renderer isolation | renderer bundle | static scan | fs/child_process/sqliteを直接importしない | `electron/tests/main_security.test.ts` |
| TC-DESKTOP-001-04 | P1 | unit | package scripts | 承認済み仕様に適合する最小入力と、必要な依存をmockした状態 | `createMainWindow(): BrowserWindow`を通じて「package scripts」を実行する | 「package scripts」の承認済み仕様を満たし、戻り値・永続化・eventが再実行可能かつ決定的である。 | `electron/tests/preload_contract.test.ts` |
| TC-DESKTOP-001-05 | P1 | unit | contextBridge | 承認済み仕様に適合する最小入力と、必要な依存をmockした状態 | `createMainWindow(): BrowserWindow`を通じて「contextBridge」を実行する | 「contextBridge」の承認済み仕様を満たし、戻り値・永続化・eventが再実行可能かつ決定的である。 | `electron/tests/main_security.test.ts` |
| TC-DESKTOP-001-06 | P1 | integration_mock | typed IPC contract | 承認済み仕様に適合する最小入力と、必要な依存をmockした状態 | `createMainWindow(): BrowserWindow`を通じて「typed IPC contract」を実行する | 「typed IPC contract」の承認済み仕様を満たし、戻り値・永続化・eventが再実行可能かつ決定的である。 | `electron/tests/preload_contract.test.ts` |
| TC-DESKTOP-001-07 | P1 | integration_mock | 任意IPC/FS/child_process非公開 | 承認済み仕様に適合する最小入力と、必要な依存をmockした状態 | `createMainWindow(): BrowserWindow`を通じて「任意IPC/FS/child_process非公開」を実行する | 「任意IPC/FS/child_process非公開」の承認済み仕様を満たし、戻り値・永続化・eventが再実行可能かつ決定的である。 | `electron/tests/main_security.test.ts` |
| TC-DESKTOP-001-08 | P0 | unit | 必須入力欠落 | 主ID、必須path、必須設定のいずれかが欠落した入力 | `createMainWindow(): BrowserWindow`を実行する | 副作用を開始する前に安定したvalidation errorを返し、既存ファイル・DB・成果物を変更しない。 | `electron/tests/preload_contract.test.ts` |
| TC-DESKTOP-001-09 | P1 | unit | 再実行時の決定性 | 同じ入力、同じ設定、同じ依存応答 | `createMainWindow(): BrowserWindow`を2回実行する | 仕様上追記が必要なversion以外は同じ論理結果を返し、重複外部呼出し・重複正式成果物を発生させない。 | `electron/tests/main_security.test.ts` |
...
```powershell
npm test -- electron/tests/main_security.test.ts electron/tests/preload_contract.test.ts
```
