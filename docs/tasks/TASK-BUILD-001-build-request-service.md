...
- TASK-DB-002
- TASK-PROJECT-001
contract_ref: docs/test-cases/TASK-BUILD-001-build-request-service.md
preparation_ref: docs/spec-proposals/task/112_task-build-001-build-request-service.md
spec_refs:
...
1. 承認済み仕様・DB・画面文書
2. `docs/test-cases/TASK-BUILD-001-build-request-service.md`
...
|---|---|---|
| `script/services/build_requests.py` | `BuildRequestService.create(command: CreateBuildRequest) -> BuildRequest` | 形式をcanonical JSON化しdraftとして保存する。 |
| `script/services/build_requests.py` | `serialize_output_formats(values) -> str` | `["mp3","text"]`等の空白なしJSONへ変換する。 |
...
|---|---|---|---|---|---|---|---|
| TC-BUILD-001-01 | P0 | unit | text-only | formats=[text], voiceなし | Build Requestを作成 | draftとして成功する | `tests/test_build_request_service.py` |
| TC-BUILD-001-02 | P0 | unit | mp3+text | formats=[text,mp3], voiceあり | 作成する | JSONは["mp3","text"]になる | `tests/test_build_request_service.py` |
| TC-BUILD-001-03 | P0 | unit | 空・未知・重複 | 空、epub、mp3重複をそれぞれ渡す | 作成する | 副作用前に拒否する | `tests/test_build_request_service.py` |
| TC-BUILD-001-04 | P1 | unit | output_formatsの1件以上/許可値/重複禁止/order | 承認済み仕様に適合する最小入力と、必要な依存をmockした状態 | `BuildRequestService.create(command: CreateBuildRequest) -> BuildRequest`を通じて「output_formatsの1件以上/許可値/重複禁止/order」を実行する | 重複を検出し、仕様で許可されない重複は安定したvalidation errorとして副作用前に拒否する。 | `tests/test_build_request_service.py` |
| TC-BUILD-001-05 | P1 | unit | Project存在確認 | 承認済み仕様に適合する最小入力と、必要な依存をmockした状態 | `BuildRequestService.create(command: CreateBuildRequest) -> BuildRequest`を通じて「Project存在確認」を実行する | 「Project存在確認」の承認済み仕様を満たし、戻り値・永続化・eventが再実行可能かつ決定的である。 | `tests/test_build_request_service.py` |
| TC-BUILD-001-06 | P0 | unit | 必須入力欠落 | 主ID、必須path、必須設定のいずれかが欠落した入力 | `BuildRequestService.create(command: CreateBuildRequest) -> BuildRequest`を実行する | 副作用を開始する前に安定したvalidation errorを返し、既存ファイル・DB・成果物を変更しない。 | `tests/test_build_request_service.py` |
| TC-BUILD-001-07 | P1 | unit | 再実行時の決定性 | 同じ入力、同じ設定、同じ依存応答 | `BuildRequestService.create(command: CreateBuildRequest) -> BuildRequest`を2回実行する | 仕様上追記が必要なversion以外は同じ論理結果を返し、重複外部呼出し・重複正式成果物を発生させない。 | `tests/test_build_request_service.py` |
| TC-BUILD-001-08 | P0 | unit | 入力・既存成果物の不変性 | hash取得済みの入力と既存正常成果物 | 正常処理または意図的な失敗を発生させる | 入力と既存正常成果物のbyte/hashが変化せず、派生物・一時物・新versionだけが変更対象になる。 | `tests/test_build_request_service.py` |
...
```powershell
python -m pytest --collect-only -q tests/test_build_request_service.py
```

### 外部接続なしのPython対象テスト

```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_build_request_service.py
```
