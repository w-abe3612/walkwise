...
- TASK-AUDIO-003
- TASK-AI-003
contract_ref: docs/test-cases/TASK-WORKER-001-python-worker-request-and-event-protocol.md
preparation_ref: docs/spec-proposals/task/141_task-worker-001-python-worker-request-and-event-protocol.md
spec_refs:
...
1. 承認済み仕様・DB・画面文書
2. `docs/test-cases/TASK-WORKER-001-python-worker-request-and-event-protocol.md`
...
|---|---|---|
| `script/worker/protocol.py` | `WorkerRequest/WorkerEvent/WorkerError` | JSON Linesのrequest/result/progress/artifact/errorを型付けする。 |
| `script/worker/handlers.py` | `HandlerRegistry.register/dispatch` | command名でhandlerを選択し未知commandを拒否する。 |
| `script/worker/cli.py` | `main(stdin, stdout) -> int` | 1行1requestを読み1行ずつeventをflushする。 |
...
|---|---|---|---|---|---|---|---|
| TC-WORKER-001-01 | P0 | integration_mock | JSON Lines | 2requestをstdinへ投入 | mainを実行 | requestごとのeventを行単位・flush順で出す | `tests/test_worker_protocol.py` |
| TC-WORKER-001-02 | P0 | unit | 未知command | 未登録command | dispatch | error eventを返しprocessを継続 | `tests/test_worker_dispatch.py` |
| TC-WORKER-001-03 | P0 | integration_mock | stdout汚染 | handlerがlogを出す | 実行 | protocol stdoutへ非JSONを出さずstderr/fileへ分離 | `tests/test_worker_protocol.py` |
| TC-WORKER-001-04 | P1 | unit | handler registry | 承認済み仕様に適合する最小入力と、必要な依存をmockした状態 | `WorkerRequest/WorkerEvent/WorkerError`を通じて「handler registry」を実行する | 「handler registry」の承認済み仕様を満たし、戻り値・永続化・eventが再実行可能かつ決定的である。 | `tests/test_worker_dispatch.py` |
| TC-WORKER-001-05 | P1 | unit | 1行1JSON | 承認済み仕様に適合する最小入力と、必要な依存をmockした状態 | `WorkerRequest/WorkerEvent/WorkerError`を通じて「1行1JSON」を実行する | 「1行1JSON」の承認済み仕様を満たし、戻り値・永続化・eventが再実行可能かつ決定的である。 | `tests/test_worker_protocol.py` |
| TC-WORKER-001-06 | P1 | unit | progress | 承認済み仕様に適合する最小入力と、必要な依存をmockした状態 | `WorkerRequest/WorkerEvent/WorkerError`を通じて「progress」を実行する | current/totalとmessageを単調・順序どおりに通知し、完了後に進捗を逆行させない。 | `tests/test_worker_dispatch.py` |
| TC-WORKER-001-07 | P1 | unit | artifact | 承認済み仕様に適合する最小入力と、必要な依存をmockした状態 | `WorkerRequest/WorkerEvent/WorkerError`を通じて「artifact」を実行する | 「artifact」の承認済み仕様を満たし、戻り値・永続化・eventが再実行可能かつ決定的である。 | `tests/test_worker_protocol.py` |
| TC-WORKER-001-08 | P0 | unit | 必須入力欠落 | 主ID、必須path、必須設定のいずれかが欠落した入力 | `WorkerRequest/WorkerEvent/WorkerError`を実行する | 副作用を開始する前に安定したvalidation errorを返し、既存ファイル・DB・成果物を変更しない。 | `tests/test_worker_dispatch.py` |
| TC-WORKER-001-09 | P1 | unit | 再実行時の決定性 | 同じ入力、同じ設定、同じ依存応答 | `WorkerRequest/WorkerEvent/WorkerError`を2回実行する | 仕様上追記が必要なversion以外は同じ論理結果を返し、重複外部呼出し・重複正式成果物を発生させない。 | `tests/test_worker_protocol.py` |
| TC-WORKER-001-10 | P0 | unit | 入力・既存成果物の不変性 | hash取得済みの入力と既存正常成果物 | 正常処理または意図的な失敗を発生させる | 入力と既存正常成果物のbyte/hashが変化せず、派生物・一時物・新versionだけが変更対象になる。 | `tests/test_worker_dispatch.py` |
...
```powershell
python -m pytest --collect-only -q tests/test_worker_protocol.py tests/test_worker_dispatch.py
```

### 外部接続なしのPython対象テスト

```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_worker_protocol.py tests/test_worker_dispatch.py
```
