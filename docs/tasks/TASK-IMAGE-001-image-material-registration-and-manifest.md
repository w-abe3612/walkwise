...
depends_on:
- TASK-INGEST-001
contract_ref: docs/test-cases/TASK-IMAGE-001-image-material-registration-and-manifest.md
preparation_ref: docs/spec-proposals/task/117_task-image-001-image-material-registration-and-manifest.md
spec_refs:
...
1. 承認済み仕様・DB・画面文書
2. `docs/test-cases/TASK-IMAGE-001-image-material-registration-and-manifest.md`
...
```powershell
python -m pytest --collect-only -q tests/test_image_ingestion.py tests/test_image_manifest.py
```

### 外部接続なしのPython対象テスト

```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_image_ingestion.py tests/test_image_manifest.py
```
