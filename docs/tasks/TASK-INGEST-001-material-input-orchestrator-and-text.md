...
- TASK-RIGHTS-001
- TASK-JOB-001
contract_ref: docs/test-cases/TASK-INGEST-001-material-input-orchestrator-and-text.md
preparation_ref: docs/spec-proposals/task/116_task-ingest-001-material-input-orchestrator-and-text.md
spec_refs:
...
1. 承認済み仕様・DB・画面文書
2. `docs/test-cases/TASK-INGEST-001-material-input-orchestrator-and-text.md`
...
```powershell
python -m pytest --collect-only -q tests/test_material_input_orchestrator.py tests/test_text_ingestion.py
```

### 外部接続なしのPython対象テスト

```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_material_input_orchestrator.py tests/test_text_ingestion.py
```
