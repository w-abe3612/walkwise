...
- TASK-PROFILE-001
- TASK-CORE-001
contract_ref: docs/test-cases/TASK-TTS-001-tts-client-interface-and-registry.md
preparation_ref: docs/spec-proposals/task/133_task-tts-001-tts-client-interface-and-registry.md
spec_refs:
...
1. 承認済み仕様・DB・画面文書
2. `docs/test-cases/TASK-TTS-001-tts-client-interface-and-registry.md`
...
```powershell
python -m pytest --collect-only -q tests/test_tts_client_contract.py tests/test_tts_registry.py
```

### 外部接続なしのPython対象テスト

```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_tts_client_contract.py tests/test_tts_registry.py
```
