...
- TASK-AI-001
- TASK-SOURCE-002
contract_ref: docs/test-cases/TASK-AI-002-ai-routing-cost-cache-and-budget.md
preparation_ref: docs/spec-proposals/task/125_task-ai-002-ai-routing-cost-cache-and-budget.md
spec_refs:
...
1. 承認済み仕様・DB・画面文書
2. `docs/test-cases/TASK-AI-002-ai-routing-cost-cache-and-budget.md`
...
```powershell
python -m pytest --collect-only -q tests/test_ai_routing.py tests/test_ai_cache.py tests/test_ai_budget.py
```

### 外部接続なしのPython対象テスト

```powershell
python -m pytest -ra -m "not integration_smoke and not integration_live and not performance and not resilience" tests/test_ai_routing.py tests/test_ai_cache.py tests/test_ai_budget.py
```
