...
generated_by:
  type: ai
  task: docs/tasks/07_source-storage-schema.md
  source_task: docs/spec-proposals/task/7_source-storage-and-common-schema.md
depends_on:
...
> **依存関係の注記**
> 本タスクは`docs/spec-proposals/task/2_file-persistence-operations.md`
> (ファイル保存運用の詳細: atomic write、ファイルロック、バックアップ世代数)への
> 依存を宣言しているが、`docs/tasks/`の夜間実行対象にタスク2に対応する項目
> (`docs/tasks/02_file-persistence-operations.md`)が存在しない
> (詳細は`docs/tasks/overnight-specification-run-report.md`を参照)。
> このため、ファイルロック・atomic write・バックアップ世代数といった
> **書き込み手順の詳細**は本書の対象外・未決定事項として残し、
> `17-file-based-data-persistence-policy.md`(承認済み・方針レベル)と
...
- 権利状態の詳細な遷移(→タスク08)
- データベースへの移行(`17-file-based-data-persistence-policy.md`により初期対象外)
...
- `17-file-based-data-persistence-policy.md`の方針(DB非使用、YAML/JSON/Markdown中心、
