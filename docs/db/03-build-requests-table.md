...
| `voice_profile_id` | text | null | `null` | FK → `voice_profiles.voice_profile_id`(`06-voice-profiles-table.md`、MVPはVOICEVOXのみ)。`output_formats_json`が`mp3`を含まない場合は`null`可 |
...
- FK: `voice_profile_id` → `voice_profiles.voice_profile_id`(`06-voice-profiles-table.md`)
...
- `voice_profile_id`を指定する場合、参照先は同一Projectに属し(`voice_profile_project_mismatch`)、
  `status = approved`でなければならない(`voice_profile_not_approved`/
  `voice_profile_archived`)。draft/archivedのVoiceProfileは新規Build参照から
  拒否する(`06-voice-profiles-table.md` 4.2節、`script/services/build_requests.py`)。
...
| `mp3`を含むのに`voice_profile_id`が`null` | DB制約またはApplication Service層で拒否する(`voice_profile_required`) |
| 存在しない`voice_profile_id`を参照する | FK制約違反として拒否する |
| 別Projectの`voice_profile_id`を参照する | Application Service層で拒否する(`voice_profile_project_mismatch`) |
| draft/archived状態の`voice_profile_id`を参照する | Application Service層で拒否する(`voice_profile_not_approved`/`voice_profile_archived`) |
...
- 将来、`mp3`/`text`以外の出力形式(EPUB等)を追加する場合、
  ...
  既存行の値は変更しない。
- `voice_profile_id`への`voice_profiles`テーブルFK追加(TASK-BUILD-EXEC-001、
  `0002_voice_profiles_and_build_execution.sql`)は、SQLiteが既存列への
  `ALTER TABLE ADD CONSTRAINT`を提供しないため、テーブル再作成パターン
  (新テーブル作成→列指定`INSERT ... SELECT`→旧テーブルdrop→rename)で行った。
  適用前に`voice_profile_id`の参照先不在行がないことを事前確認する
  (`06-voice-profiles-table.md` 9節)。
