...
次の5つを**製品ドメインテーブル**とする。利用者の作品データを表し、
5テーブルという制限の対象である。
...
アプリ初回起動時、空のSQLiteファイルが作成され、初期migrationにより
5つの製品ドメインテーブル(projects/sources/build_requests/jobs/artifacts)と
1つの内部システム管理テーブル(schema_migrations)が作成される。
