...
`voice_profiles`は、TASK-BUILD-EXEC-001(2026-07-22、人間承認済み設計)により
5節の昇格手順を経て追加された6つ目の製品ドメインテーブルである(旧来は
`voice_profile_refs`としてJSON列・親テーブル最小列での保持を想定していたが、
FK・UNIQUE制約・独立したstatusライフサイクル(draft/approved/archived)が
必要と判断され、専用テーブルへ昇格した。詳細は`06-voice-profiles-table.md`、
`00-database-overview.md` 5.13節)。

`job_events`、`source_revisions`、`output_profiles`、`approval_records`、
`app_settings`は、引き続きMVPでは別テーブルにせず、JSON列・ファイル・
親テーブルの最小列で保持する。検索・制約上の必要性が確認された場合のみ、
5節の昇格手順に従ってテーブルを追加する。

## 8. 製品ドメインテーブルと内部システム管理テーブル

`01`〜`06`(製品ドメインテーブル)と`90`番台(内部システム管理テーブル)は
役割が異なる。

| 区分 | 対象 | 6テーブル制限 | archive/delete規則 | 利用者からの可視性 |
|---|---|---|---|---|
| 製品ドメインテーブル(`01`〜`06`) | projects/sources/build_requests/jobs/artifacts/voice_profiles | 対象 | `01-projects-table.md`のarchive規則に従う(`voice_profiles`は`06-voice-profiles-table.md`) | 画面から参照される |
| 内部システム管理テーブル(`90`番台) | schema_migrations | 対象外 | 適用しない(追記専用) | 利用者は直接参照しない |

内部システム管理テーブルを追加しても、7節の「製品ドメインテーブルは6つ」という
