...
> **状態に関する注記**
> `19-application-scope-and-mvp.md`により、EPUB入力はpost-MVPの承認済み
> 仕様である。MVP画面には表示しない。EPUB出力機能(音声・テキスト以外の
> 出力形式としてのEPUB)とは別物であり、本書はSourceとしてのEPUB入力のみを扱う。
...
- Kindle操作・Kindleキャプチャ(製品の恒久的対象外、`19-application-scope-and-mvp.md`)

## 4. 現行実装

現行コードにEPUB抽出の実装は存在しない。本書は仕様として承認済みであるが、
実装コードは現時点では存在しない(post-MVPのため実装優先度も低い)。

## 5. 推奨仕様

### 5.1 対応EPUBとDRM

DRMなしEPUB 2/3を対象とし、DRM回避は対象外とする。
...
package documentのspineを本文順の正本とし、
...
(`source-preprocessing.md`の前処理で読み表記変換を行う際の入力になる)。
脚注は本文中の参照位置を保持し、本文へ無条件で埋め込まない
(読み上げ順序を暗黙に変えない)。

### 5.4 CSSと装飾

意味を持つ構造だけを残し、表示専用CSSは正本にしない。
...
- 画像内テキスト(図中の文字)をOCR対象とするかどうか(`ocr-and-scanned-pdf.md`との境界)
