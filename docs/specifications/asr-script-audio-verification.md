...
> **状態に関する注記**
> `19-application-scope-and-mvp.md`により、ASR照合はpost-MVPの任意review支援
> 機能として承認済みである。必須の振る舞い(任意機能である位置付け、比較対象、
> 自動fail禁止等)は本書で確定する。ASRエンジンの最終選定・具体的な誤検出率は
> benchmark実施後に確定するため、`provisional`として区別する。本機能は
> 録音素材の取り込みを意味せず、TTS成果物の検査に限定される
> (`19-application-scope-and-mvp.md` 5.5節の恒久的対象外とは無関係)。
...
- ASRの実行・benchmark(実装時に実施)
- 発音・声質・キャラクター適合の最終判定(常に人間、`13-audio-validation.md`と同様)
- ASR単独での正式成果物の自動合否判定

## 4. 現行実装

現行コードにASR照合の実装は存在しない。本書は仕様として承認済みであるが、実装コードは現時点では存在しない。

## 5. 推奨仕様

### 5.1 必須工程にするか

必須にせず、追加のreview支援機能にする。
...
provider interfaceを定義し、初期候補はローカルWhisper互換エンジンと
...
文字誤り率(CER)、単語誤り率(WER)、欠落・重複・順序差を使い、
...
自動fail可否は「しない」とする。明確な空音声・順序欠落など、ASR以外でも
...
既定では送らない。外部送信は明示許可、rights、privacy記録が
ある場合だけにする。**
...
- [x] 正常な言い換えや発音差を過剰検出しない方針が明示されている(実測は実装時)。
- [x] 外部送信の有無とデータ取り扱いが明示されている。
- [x] ASRなしでも基本パイプラインが動作する。
- [x] post-MVPの任意review支援機能として承認されている。
- [x] 未実測のASRエンジン・閾値は`provisional`として区別されている(9, 15節)。
- [x] 実装コードを変更していない。

## 17. 実装時の確認事項

- 音声サンプルによるbenchmark実施後に最終ASRエンジンを確定する。
- 外部API送信は既定で無効化する(5.6節)。許可は実装・運用時に個別確認する。
- ASR差分だけで正式成果物を自動failにしない(5.5節で明示的に禁止)。
