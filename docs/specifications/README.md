...
- [PDF直接テキスト抽出](pdf-direct-text-extraction.md)(MVP必須)
- [OCRと画像・スキャンPDF](ocr-and-scanned-pdf.md)(MVP必須)
- [書籍データ前処理](source-preprocessing.md)
- [EPUBテキスト抽出](epub-text-extraction.md)(post-MVP)

Kindle操作・Kindleキャプチャ・Kindle専用ツールの開発、動画・録音音声の資料取り込みは、
製品の恒久的対象外である(`19-application-scope-and-mvp.md` 5.5節)。これらの
未承認提案は存在せず、今後も作成しない。
...
- [M4B出力](m4b-output.md)(post-MVP、全文MP3は作成しない)
- [ASRによる原稿と音声の照合](asr-script-audio-verification.md)(post-MVP、任意review支援)
...
- COEIROINKクライアント(`status: blocked`。公式API・実機情報が未確認)
- 話者別の最終プロファイル値
- 音声検査の最終閾値(`status: provisional`。実測未了)

Kindle画面キャプチャ、Kindle専用ツール、動画・授業録音の資料入力は、
未承認提案としても存在しない(製品の恒久的対象外、`19-application-scope-and-mvp.md` 5.5節)。
